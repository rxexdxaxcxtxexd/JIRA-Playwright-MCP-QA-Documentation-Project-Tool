...
test.describe('Commodity - Features & Validation', () => {
    let consoleLogs = [];
    let consoleErrors = [];
    ...
    test('Page Title is "Commoditys"', async ({ page }) => {
        await page.goto(`${baseUrl}/Commodity/Index`);
        await page.waitForLoadState('networkidle');

        // Check page heading
        const heading = page.locator('h1:has-text("Commoditys")');
    ...
    test('Search Form Has Correct Fields', async ({ page }) => {
        ...
        const accountingCodeField = page.locator('#CommodityCode');
    ...
    test('Search Results Table Has Correct Column Headers', async ({ page }) => {
        ...
        const headers = await page.locator('#commodityTable thead th').allTextContents();
    ...
    test('Add Commodity Button Exists', async ({ page }) => {
        await page.goto(`${baseUrl}/Commodity/Index`);
        await page.waitForLoadState('networkidle');

        // Check for Add Commodity button
        const addButton = page.locator('a.btn-outline-primary:has-text("Add Commodity")');
        await expect(addButton).toBeVisible();

        // Verify it links to Create action
        const href = await addButton.getAttribute('href');
        expect(href).toContain('/Commodity/Create');
    });
    ...
    test('Create Form Has Required Field Indicators', async ({ page }) => {
        ...
        // Check for required field (Commodity Name)
    ...
    test('Edit Form Loads Existing Commodity Data', async ({ page }) => {
        await page.goto(`${baseUrl}/Commodity/Index`);
        await page.waitForLoadState('networkidle');
        await page.waitForTimeout(1000);
    ...
    test('Details Page Shows Read-Only Information', async ({ page }) => {
        ...
        if (await firstViewButton.count() > 0) {
            ...
            // Verify details page shows commodity information
    ...
    test('Form Layout Matches Design', async ({ page }) => {
        ...
        await page.screenshot({ path: 'test-results/commodity-features-form-layout.png', fullPage: true });
    });
    ...
    test('DataTable Has Search Box', async ({ page }) => {
        ...
        const searchBox = page.locator('#commodityTable_filter input');
    ...
    test('DataTable Has Pagination Controls', async ({ page }) => {
        ...
        const pagination = page.locator('#commodityTable_paginate');
