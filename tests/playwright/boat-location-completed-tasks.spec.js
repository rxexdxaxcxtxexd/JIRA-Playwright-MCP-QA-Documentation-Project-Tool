...
test.describe('Boat Location - Completed Tasks Verification', () => {
    ...
    // ============================================
    // TASK 2: Verify HP and Business Unit Columns are Combined
    // ============================================
    test('TASK 2: Search results table has combined HP Business Unit column', async ({ page }) => {
        await page.goto(`${baseUrl}/BoatLocationSearch/Index`);
        await page.waitForLoadState('networkidle');
        ...
        // Check for combined "HP Business Unit" column header (may be hidden by DataTables responsive)
        const hpBusinessUnitHeader = page.locator('#boatLocationTable thead th:has-text("HP Business Unit")').first();
        await expect(hpBusinessUnitHeader).toBeAttached(); // Just check it exists in DOM

        // Verify separate "HP" column does NOT exist
        const separateHPHeader = page.locator('#boatLocationTable thead th:has-text("HP")').filter({ hasNotText: 'Business Unit' });
        await expect(separateHPHeader).toHaveCount(0);

        // Verify separate "Business Unit" column does NOT exist
        const separateBusinessUnitHeader = page.locator('#boatLocationTable thead th:has-text("Business Unit")').filter({ hasNotText: 'HP' });
        await expect(separateBusinessUnitHeader).toHaveCount(0);

        // Verify the combined column exists in the table (check the actual count)
        const allHeaders = await page.locator('#boatLocationTable thead th').allTextContents();
        const hasHPBusinessUnit = allHeaders.some(h => h.trim() === 'HP Business Unit');
        expect(hasHPBusinessUnit).toBeTruthy();
    });
