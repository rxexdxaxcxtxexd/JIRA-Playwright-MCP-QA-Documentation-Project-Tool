...
test.describe('CommoditySearch - Comprehensive Form Behavior Analysis', () => {
    let testReport = {
        testSuite: 'CommoditySearch Form Behavior',
        timestamp: new Date().toISOString(),
        baseURL: 'https://localhost:6001/Commodity/Index',
        scenarios: []
    };
    ...
    test.beforeEach(async ({ page }) => {
        ...
        page.on('request', request => {
            if (request.url().includes('CommodityTable') || request.url().includes('Commodity')) {
        ...
        page.on('response', async response => {
            if (response.url().includes('CommodityTable') || response.url().includes('Commodity')) {
                const responseData = {
                    url: response.url(),
    ...
    test.afterAll(async () => {
        const reportPath = path.join(__dirname, '..', 'test-results', 'CommoditySearch_FORM_BEHAVIOR_REPORT.md');

        let markdown = `# CommoditySearch Form Behavior Report\n\n`;
    ...
    test('Scenario 1: Empty Submission - All Fields Blank', async ({ page }) => {
        ...
        const resultsTable = page.locator('#commodityTable');
        ...
        if (tableVisible) {
            ...
            rowCount = await page.locator('#commodityTable tbody tr').count();

            if (rowCount > 0) {
                const firstCell = await page.locator('#commodityTable tbody tr td').first();
    ...
    test('Scenario 2: Search by Commodity Name - Exact Match', async ({ page }) => {
        const scenarioName = 'Search by Name - Exact Match';
        const description = 'Search for a commodity by exact name match to verify search functionality works correctly.';
        ...
        const tableData = await page.evaluate(() => {
            const rows = Array.from(document.querySelectorAll('#commodityTable tbody tr'));
            return rows.map(row => {
        ...
        await page.screenshot({ path: 'test-results/commodity-scenario2-exact-match.png', fullPage: true });
        captureScenarioResult(scenarioName, description, inputs, observations);
    });

    test('Scenario 3: Search by Commodity Name - Partial Match', async ({ page }) => {
        ...
        await page.screenshot({ path: 'test-results/commodity-scenario3-partial-match.png', fullPage: true });
        captureScenarioResult(scenarioName, description, inputs, observations);
    });
    ...
    test('Scenario 4: Search by Description', async ({ page }) => {
        ...
        await page.screenshot({ path: 'test-results/commodity-scenario4-description.png', fullPage: true });
        captureScenarioResult(scenarioName, description, inputs, observations);
    });
    ...
    test('Scenario 5: Active Only Filter', async ({ page }) => {
        ...
        const activeOnlyRowCount = await page.locator('#commodityTable tbody tr').count();
        ...
        const allRowCount = await page.locator('#commodityTable tbody tr').count();
        ...
        await page.screenshot({ path: 'test-results/commodity-scenario5-active-filter.png', fullPage: true });
        captureScenarioResult(scenarioName, description, inputs, observations);
    });
    ...
    test('Scenario 6: DataTable Pagination', async ({ page }) => {
        ...
        await page.waitForSelector('#commodityTable', { state: 'visible' });
        await page.waitForTimeout(2000);

        const paginationControls = page.locator('#commodityTable_paginate');
        ...
        const observations = {
            ...
            tableInitialized: await page.locator('#commodityTable').count() > 0
        };

        await page.screenshot({ path: 'test-results/commodity-scenario6-pagination.png', fullPage: true });
        captureScenarioResult(scenarioName, description, inputs, observations);
    });
    ...
    test('Scenario 7: DataTable Sorting', async ({ page }) => {
        ...
        const nameHeader = page.locator('#commodityTable thead th').nth(2); // Name column
        ...
        const observations = {
            ...
            tableStillFunctional: await page.locator('#commodityTable').count() > 0
        };

        await page.screenshot({ path: 'test-results/commodity-scenario7-sorting.png', fullPage: true });
        captureScenarioResult(scenarioName, description, inputs, observations);
    });
    ...
    test('Scenario 8: SQL Injection & XSS Attempts', async ({ page }) => {
        ...
        await page.screenshot({ path: 'test-results/commodity-scenario8-security-testing.png', fullPage: true });
        captureScenarioResult(scenarioName, description, inputs, observations);
    });
    ...
    test('Scenario 9: Rapid Form Submission - Multiple Clicks', async ({ page }) => {
        ...
        await page.screenshot({ path: 'test-results/commodity-scenario9-rapid-submission.png', fullPage: true });
        captureScenarioResult(scenarioName, description, inputs, observations);
    });
});
