...
test.describe('CommodityEdit - Delete/Deactivate Workflows', () => {
    let consoleLogs = [];
    let consoleErrors = [];
    ...
    test('Toggle Active Status - Deactivate Commodity', async ({ page }) => {
        console.log('\n========================================');
        ...
        // Get first commodity in results
        ...
        if (!hasResults) {
            console.log('⚠️  No active commoditys found to test deactivation');
        ...
        if (await editButton.count() > 0) {
            ...
            const commodityIdMatch = currentUrl.match(/\/Commodity\/Details\/(\d+)/);
            if (commodityIdMatch) {
                await page.goto(`${baseUrl}/Commodity/Edit/${commodityIdMatch[1]}`);
                await page.waitForLoadState('networkidle');
                await page.waitForTimeout(2000);
            }
        }

        // Get commodity name for verification
        const commodityNameField = page.locator('input[name="Name"]');
        const commodityName = await commodityNameField.inputValue();
        console.log(`Testing deactivation for: ${commodityName}`);
        ...
        await page.screenshot({ path: 'test-results/commodity-delete-before-toggle.png', fullPage: true });
        ...
        if (initialCheckedState) {
            ...
            await page.screenshot({ path: 'test-results/commodity-delete-after-toggle.png', fullPage: true });

            console.log('✅ Commodity deactivated successfully');
        } else {
            console.log('Commodity already inactive');
        }
    ...
    test('Toggle Active Status - Reactivate Commodity', async ({ page }) => {
        console.log('\n========================================');
        ...
        // Search for inactive commoditys (set ActiveOnly to false)
        ...
        const allRows = await page.locator('#commodityTable tbody tr').count();
        console.log(`Total rows found: ${allRows}`);

        if (allRows === 0) {
            console.log('⚠️  No commoditys found to test reactivation');
            expect(true).toBeTruthy();
            return;
        }
        ...
        const commodityNameField = page.locator('input[name="Name"]');
        const commodityName = await commodityNameField.inputValue();
        console.log(`Testing commodity: ${commodityName}`);
        ...
        if (!isChecked) {
            console.log('Found inactive commodity - attempting to reactivate...');
            ...
            await page.screenshot({ path: 'test-results/commodity-delete-reactivate.png', fullPage: true });

            expect(afterChecked).toBeTruthy();
        } else {
            console.log('Commodity is already active');
        }
    ...
    test('Verify Active Status Filter Behavior', async ({ page }) => {
        ...
        await page.waitForSelector('#commodityTable tbody tr', { timeout: 10000 });
        const activeRowCount = await page.locator('#commodityTable tbody tr').count();
        console.log(`Active commoditys found: ${activeRowCount}`);
        ...
        console.log(`All commoditys found: ${allRowCount}`);

        await page.screenshot({ path: 'test-results/commodity-delete-filter-behavior.png', fullPage: true });
    ...
    test('Cancel Button Returns to Index', async ({ page }) => {
        ...
        if (await firstViewButton.count() > 0) {
            ...
            const onIndexPage = currentUrl.includes('/Index') || currentUrl.includes('/Commodity');
            expect(onIndexPage).toBeTruthy();

            await page.screenshot({ path: 'test-results/commodity-delete-cancel-navigation.png', fullPage: true });
