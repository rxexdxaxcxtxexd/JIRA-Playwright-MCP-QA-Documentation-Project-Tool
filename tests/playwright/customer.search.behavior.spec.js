...
test.describe('CustomerSearch - Comprehensive Form Behavior Analysis', () => {
    let testReport = {
        testSuite: 'CustomerSearch Form Behavior',
        timestamp: new Date().toISOString(),
        baseURL: 'https://localhost:6001/Customer/Index',
        scenarios: []
    };
    ...
    test.beforeEach(async ({ page }) => {
        ...
        page.on('request', request => {
            if (request.url().includes('CustomerTable') || request.url().includes('Customer')) {
        ...
        page.on('response', async response => {
            if (response.url().includes('CustomerTable') || response.url().includes('Customer')) {
                const responseData = {
                    url: response.url(),
    ...
    test.afterAll(async () => {
        const reportPath = path.join(__dirname, '..', 'test-results', 'CustomerSearch_FORM_BEHAVIOR_REPORT.md');

        let markdown = `# CustomerSearch Form Behavior Report\n\n`;
    ...
    test('Scenario 1: Empty Submission - All Fields Blank', async ({ page }) => {
        ...
        const resultsTable = page.locator('#customerTable');
        ...
        if (tableVisible) {
            ...
            rowCount = await page.locator('#customerTable tbody tr').count();

            if (rowCount > 0) {
                const firstCell = await page.locator('#customerTable tbody tr td').first();
    ...
    test('Scenario 2: Search by Customer Name - Exact Match', async ({ page }) => {
        const scenarioName = 'Search by Name - Exact Match';
        const description = 'Search for a customer by exact name match to verify search functionality works correctly.';
        ...
        const rowCount = await page.locator('#customerTable tbody tr').count();
        ...
        await page.screenshot({ path: 'test-results/customer-scenario2-exact-match.png', fullPage: true });
        captureScenarioResult(scenarioName, description, inputs, observations);
    });

    test('Scenario 3: Search by Customer Name - Partial Match', async ({ page }) => {
        const scenarioName = 'Search by Name - Partial Match';
        const description = 'Search for customers using partial name match to verify wildcard search functionality.';
        ...
        await page.screenshot({ path: 'test-results/customer-scenario3-partial-match.png', fullPage: true });
        captureScenarioResult(scenarioName, description, inputs, observations);
    });

    test('Scenario 4: Search by Accounting Code', async ({ page }) => {
        const scenarioName = 'Search by Accounting Code';
        const description = 'Search for customers by accounting code to verify code-based search functionality.';

        const accountingCodeInput = page.locator('#AccountingCode');
        const findButton = page.locator('#btnSearch');

        const inputs = {
            Name: '',
            AccountingCode: 'PWTST',
            ActiveOnly: 'true'
        };

        await accountingCodeInput.fill(inputs.AccountingCode);
        await page.waitForTimeout(500);
        ...
        await page.screenshot({ path: 'test-results/customer-scenario4-accounting-code.png', fullPage: true });
        captureScenarioResult(scenarioName, description, inputs, observations);
    });
    ...
    test('Scenario 5: Active Only Filter', async ({ page }) => {
        ...
        const activeOnlyRowCount = await page.locator('#customerTable tbody tr').count();
        ...
        const allRowCount = await page.locator('#customerTable tbody tr').count();
        ...
        await page.screenshot({ path: 'test-results/customer-scenario5-active-filter.png', fullPage: true });
        captureScenarioResult(scenarioName, description, inputs, observations);
    });
    ...
    test('Scenario 6: DataTable Pagination', async ({ page }) => {
        ...
        const paginationControls = page.locator('#customerTable_paginate');
        ...
        const observations = {
            ...
            tableInitialized: await page.locator('#customerTable').count() > 0
        };

        await page.screenshot({ path: 'test-results/customer-scenario6-pagination.png', fullPage: true });
        captureScenarioResult(scenarioName, description, inputs, observations);
    });
    ...
    test('Scenario 7: DataTable Sorting', async ({ page }) => {
        ...
        const nameHeader = page.locator('#customerTable thead th').nth(2); // Name column
        ...
        const observations = {
            ...
            tableStillFunctional: await page.locator('#customerTable').count() > 0
        };

        await page.screenshot({ path: 'test-results/customer-scenario7-sorting.png', fullPage: true });
        captureScenarioResult(scenarioName, description, inputs, observations);
    });
    ...
    test('Scenario 8: SQL Injection & XSS Attempts', async ({ page }) => {
        ...
        const inputs = {
            ...
            AccountingCode: "<script>alert('XSS')</script>",
            ActiveOnly: 'true'
        };
        ...
        const observations = {
            ...
            xssAttempt: inputs.AccountingCode,
            formSubmitted: networkRequests.length > 0,
            serverResponse: networkResponses.length > 0,
            xssVulnerability: {
        ...
        await page.screenshot({ path: 'test-results/customer-scenario8-security-testing.png', fullPage: true });
        captureScenarioResult(scenarioName, description, inputs, observations);
    });
    ...
    test('Scenario 9: Rapid Form Submission - Multiple Clicks', async ({ page }) => {
        ...
        await page.screenshot({ path: 'test-results/customer-scenario9-rapid-submission.png', fullPage: true });
        captureScenarioResult(scenarioName, description, inputs, observations);
    });
});
