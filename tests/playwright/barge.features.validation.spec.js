...
test.describe('Barge - Features & Validation', () => {
    let consoleLogs = [];
    let consoleErrors = [];
    ...
    test('Page Title is "Barges"', async ({ page }) => {
        await page.goto(`${baseUrl}/BargeSearch/Index`);
        await page.waitForLoadState('networkidle');

        // Check page heading
        const heading = page.locator('h1:has-text("Barges")');
    ...
    test('Search Form Has Correct Fields', async ({ page }) => {
        ...
        const accountingCodeField = page.locator('#BargeCode');
    ...
    test('Search Results Table Has Correct Column Headers', async ({ page }) => {
        ...
        const headers = await page.locator('#bargeTable thead th').allTextContents();
    ...
    test('Add Barge Button Exists', async ({ page }) => {
        await page.goto(`${baseUrl}/BargeSearch/Index`);
        await page.waitForLoadState('networkidle');

        // Check for Add Barge button
        const addButton = page.locator('a.btn-outline-primary:has-text("Add Barge")');
        await expect(addButton).toBeVisible();

        // Verify it links to Create action
        const href = await addButton.getAttribute('href');
        expect(href).toContain('/Barge/Create');
    });
    ...
    test('Create Form Has Required Field Indicators', async ({ page }) => {
        ...
        // Check for required field (Barge Name)
    ...
    test('Edit Form Loads Existing Barge Data', async ({ page }) => {
        await page.goto(`${baseUrl}/BargeSearch/Index`);
        await page.waitForLoadState('networkidle');
        await page.waitForTimeout(1000);
    ...
    test('Details Page Shows Read-Only Information', async ({ page }) => {
        ...
        if (await firstViewButton.count() > 0) {
            ...
            // Verify details page shows barge information
    ...
    test('Form Layout Matches Design', async ({ page }) => {
        ...
        await page.screenshot({ path: 'test-results/barge-features-form-layout.png', fullPage: true });
    });
    ...
    test('DataTable Has Search Box', async ({ page }) => {
        ...
        const searchBox = page.locator('#bargeTable_filter input');
    ...
    test('DataTable Has Pagination Controls', async ({ page }) => {
        ...
        const pagination = page.locator('#bargeTable_paginate');
