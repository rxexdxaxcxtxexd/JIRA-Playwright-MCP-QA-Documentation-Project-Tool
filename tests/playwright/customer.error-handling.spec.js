...
test.describe('Customer - Error Handling & Edge Cases', () => {
    ...
    test('Handle 404 - Nonexistent Customer ID', async ({ page }) => {
        console.log('\n--- TEST: Handle 404 - Nonexistent Customer ID ---');

        // Try to navigate to a customer that doesn't exist
        const nonexistentId = 9999999;
        const response = await page.goto(`${baseUrl}/Customer/Edit/${nonexistentId}`, {
        ...
        await page.screenshot({ path: 'test-results/customer-error-404-nonexistent-id.png', fullPage: true });
    ...
    test('Handle Server Validation Errors', async ({ page }) => {
        ...
        // Leave required Customer Name empty
        ...
        await page.screenshot({ path: 'test-results/customer-error-server-validation.png', fullPage: true });
    ...
    test('Handle Empty Search Results Gracefully', async ({ page }) => {
        ...
        // Search for a customer that definitely doesn't exist
        ...
        const tableBody = page.locator('#customerTable tbody');
        const noDataMessage = page.locator('#customerTable tbody td:has-text("No data available")');
        ...
        await page.screenshot({ path: 'test-results/customer-error-empty-search.png', fullPage: true });
    ...
    test('Handle Special Characters in Search', async ({ page }) => {
        ...
        for (const char of specialChars) {
            ...
            const pageNotCrashed = await page.locator('#customerTable').count() > 0;
            expect(pageNotCrashed).toBeTruthy();
        }

        await page.screenshot({ path: 'test-results/customer-error-special-chars.png', fullPage: true });
    });
    ...
    test('Handle SQL Injection Attempts', async ({ page }) => {
        ...
        await page.screenshot({ path: 'test-results/customer-error-sql-injection.png', fullPage: true });
    });
    ...
    test('Handle XSS Attempts', async ({ page }) => {
        ...
        await page.screenshot({ path: 'test-results/customer-error-xss.png', fullPage: true });
    });
    ...
    test('Handle Concurrent Form Submissions', async ({ page }) => {
        ...
        page.on('request', request => {
            if (request.url().includes('CustomerTable')) {
                requestCount++;
        ...
        await page.screenshot({ path: 'test-results/customer-error-concurrent-submit.png', fullPage: true });

        // Should handle gracefully (ideally only 1 request, but multiple is okay if handled)
        expect(requestCount).toBeGreaterThanOrEqual(1);

        // Page should still be functional
        const tableExists = await page.locator('#customerTable').count() > 0;
    ...
    test('Handle Browser Back Button', async ({ page }) => {
        ...
        if (await firstViewButton.count() > 0) {
            ...
            await page.screenshot({ path: 'test-results/customer-error-back-button.png', fullPage: true });
    ...
    test('Handle Max Length Validation', async ({ page }) => {
        ...
        // Try to enter value exceeding max length for Customer Name (max 100)
        const nameField = page.locator('input[name="Name"]');
        ...
        await page.screenshot({ path: 'test-results/customer-error-max-length.png', fullPage: true });
