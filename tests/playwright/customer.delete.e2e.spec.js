...
test.describe('CustomerEdit - Delete/Deactivate Workflows', () => {
    let consoleLogs = [];
    let consoleErrors = [];
    ...
    test('Toggle Active Status - Deactivate Customer', async ({ page }) => {
        console.log('\n========================================');
        ...
        if (!hasResults) {
            console.log('⚠️  No active customers found to test deactivation');
        ...
        const customerNameField = page.locator('input[name="Name"]');
        const customerName = await customerNameField.inputValue();
        console.log(`Testing deactivation for: ${customerName}`);
        ...
        await page.screenshot({ path: 'test-results/customer-delete-before-toggle.png', fullPage: true });
        ...
        if (initialCheckedState) {
            ...
            await page.screenshot({ path: 'test-results/customer-delete-after-toggle.png', fullPage: true });

            console.log('✅ Customer deactivated successfully');
        } else {
            console.log('Customer already inactive');
        }
    ...
    test('Toggle Active Status - Reactivate Customer', async ({ page }) => {
        console.log('\n========================================');
        ...
        // Search for inactive customers (set ActiveOnly to false)
        ...
        const allRows = await page.locator('#customerTable tbody tr').count();
        console.log(`Total rows found: ${allRows}`);

        if (allRows === 0) {
            console.log('⚠️  No customers found to test reactivation');
            expect(true).toBeTruthy();
            return;
        }
        ...
        if (!isChecked) {
            console.log('Found inactive customer - attempting to reactivate...');
            ...
            await page.screenshot({ path: 'test-results/customer-delete-reactivate.png', fullPage: true });

            expect(afterChecked).toBeTruthy();
        } else {
            console.log('Customer is already active');
        }
    ...
    test('Verify Active Status Filter Behavior', async ({ page }) => {
        ...
        await page.waitForSelector('#customerTable tbody tr', { timeout: 10000 });
        const activeRowCount = await page.locator('#customerTable tbody tr').count();
        console.log(`Active customers found: ${activeRowCount}`);
        ...
        console.log(`All customers found: ${allRowCount}`);

        await page.screenshot({ path: 'test-results/customer-delete-filter-behavior.png', fullPage: true });
    ...
    test('Cancel Button Returns to Index', async ({ page }) => {
        ...
        if (await firstViewButton.count() > 0) {
            ...
            const onIndexPage = currentUrl.includes('/Index') || currentUrl.includes('/Customer');
            expect(onIndexPage).toBeTruthy();

            await page.screenshot({ path: 'test-results/customer-delete-cancel-navigation.png', fullPage: true });
