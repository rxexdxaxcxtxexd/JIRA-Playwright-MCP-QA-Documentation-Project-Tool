...
test.describe('Barge - Error Handling & Edge Cases', () => {
    let consoleLogs = [];
    let consoleErrors = [];
    ...
    test('Handle 404 - Nonexistent Barge ID', async ({ page }) => {
        console.log('\n--- TEST: Handle 404 - Nonexistent Barge ID ---');

        // Try to navigate to a barge that doesn't exist
        const nonexistentId = 9999999;
        const response = await page.goto(`${baseUrl}/Barge/Edit/${nonexistentId}`, {
        ...
        await page.screenshot({ path: 'test-results/barge-error-404-nonexistent-id.png', fullPage: true });
    ...
    test('Handle Server Validation Errors', async ({ page }) => {
        ...
        await page.screenshot({ path: 'test-results/barge-error-server-validation.png', fullPage: true });
    ...
    test('Handle Empty Search Results Gracefully', async ({ page }) => {
        ...
        // Search for a barge that definitely doesn't exist
        ...
        const tableBody = page.locator('#bargeTable tbody');
        const noDataMessage = page.locator('#bargeTable tbody td:has-text("No data available")');
        ...
        await page.screenshot({ path: 'test-results/barge-error-empty-search.png', fullPage: true });
    ...
    test('Handle Special Characters in Search', async ({ page }) => {
        ...
        for (const char of specialChars) {
            ...
            const pageNotCrashed = await page.locator('#bargeTable').count() > 0;
            expect(pageNotCrashed).toBeTruthy();
        }

        await page.screenshot({ path: 'test-results/barge-error-special-chars.png', fullPage: true });
    });
    ...
    test('Handle SQL Injection Attempts', async ({ page }) => {
        ...
        const sqlInjectionAttempts = [
            "'; DROP TABLE Barges; --",
            "1' OR '1'='1",
            "Robert'); DELETE FROM Barges WHERE 1=1; --"
        ];
        ...
        await page.screenshot({ path: 'test-results/barge-error-sql-injection.png', fullPage: true });
    });
    ...
    test('Handle XSS Attempts', async ({ page }) => {
        ...
        await page.screenshot({ path: 'test-results/barge-error-xss.png', fullPage: true });
    });
    ...
    test('Handle Concurrent Form Submissions', async ({ page }) => {
        ...
        page.on('request', request => {
            if (request.url().includes('BargeTable')) {
                requestCount++;
        ...
        await page.screenshot({ path: 'test-results/barge-error-concurrent-submit.png', fullPage: true });

        // Should handle gracefully (ideally only 1 request, but multiple is okay if handled)
        expect(requestCount).toBeGreaterThanOrEqual(1);

        // Page should still be functional
        const tableExists = await page.locator('#bargeTable').count() > 0;
    ...
    test('Handle Browser Back Button', async ({ page }) => {
        ...
        if (await firstViewButton.count() > 0) {
            ...
            await page.screenshot({ path: 'test-results/barge-error-back-button.png', fullPage: true });
    ...
    test('Handle Max Length Validation', async ({ page }) => {
        ...
        // Try to enter value exceeding max length for Barge Name (max 100)
        const nameField = page.locator('input[name="Name"]');
        ...
        await page.screenshot({ path: 'test-results/barge-error-max-length.png', fullPage: true });
